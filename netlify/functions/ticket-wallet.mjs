...
async function passAssets(passJson) {
  const entries = await Promise.all(ASSET_NAMES.map(async (name) => [name, await readFile(new URL(`./wallet-assets/${name}`, import.meta.url))]));
  return { ...Object.fromEntries(entries), 'pass.json': Buffer.from(JSON.stringify(passJson)) };
}
...
export default async (req) => {
  ...
  if (!order || order.status !== 'paid' || order.ticketId !== parsed.ticketId || !['paid', 'checked_in'].includes(review?.ticketState)) {
    return response('This ticket is inactive.', 403);
  }
  ...
  try {
    ...
    await writeAudit('ticket.apple_wallet_downloaded', { submissionId: parsed.submissionId, ticketId: parsed.ticketId });
    return new Response(output, {
      status: 200,
      headers: {
        'Cache-Control': 'no-store',
        'Content-Type': 'application/vnd.apple.pkpass',
        'Content-Disposition': `attachment; filename="NOCTURNE-${parsed.ticketId}.pkpass"`,
