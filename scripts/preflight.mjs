...
const ticketViewSource = await readFile(join(root, 'netlify/functions/ticket-view.mjs'), 'utf8');
...
if (!ticketViewSource.includes("from './_venue.mjs'")) failures.push('ticket-view.mjs must load private venue details from runtime configuration.');
...
for (const [name, text] of [['ticket-view.mjs', ticketViewSource], ['_apple-wallet.mjs', walletSource]]) {
...
const privatePages = ['invite.html', 'admin.html', 'admin-launch.html', 'check-in.html', 'bar.html', 'application-received.html'];
...
console.log(`NOCTURNE preflight passed: ${publicTextFiles.length} public text assets checked; private venue details are runtime-only.`);
