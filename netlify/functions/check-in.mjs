...
async function checkIn(req, input) {
  ...
  if (summary.checkedInAt) return json({ ok: true, result: 'already_used', message: 'ALREADY CHECKED IN', ticketId: parsed.ticketId, guestName, checkedInAt: summary.checkedInAt, ticketSource: gate.source, entitlements: entitlements(summary) });
  ...
  if (!write.modified) {
    ...
    return json({ ok: true, result: latest?.checkedInAt ? 'already_used' : 'conflict', message: latest?.checkedInAt ? 'ALREADY CHECKED IN' : 'SCAN AGAIN', ticketId: parsed.ticketId, guestName, checkedInAt: latest?.checkedInAt || null, ticketSource: gate.source, entitlements: entitlements(latest || summary) }, latest?.checkedInAt ? 200 : 409);
  }
