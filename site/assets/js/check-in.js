...
const lookupForm=document.querySelector('#emergency-lookup-form');
const lookupInput=document.querySelector('#emergency-lookup-input');
const lookupResults=document.querySelector('#emergency-lookup-results');
...
function entitlementLabel(data={}){const labels=[];const drink=data?.drinkPackage||{};const water=data?.waterPackage||{};const late=data?.lateStay||{};if(drink.purchased){const remaining=Number(drink.creditsRemaining||0);const purchased=Number(drink.creditsPurchased||6);labels.push(`SIX-DRINK ${remaining}/${purchased}`)}if(water.purchased)labels.push('UNLIMITED WATER');if(late.purchased)labels.push(`LATE STAY ${late.departureTime||'10:00 AM'}`);return labels.length?` · ${labels.join(' · ')}`:''}

async function validateTicket(value){const normalized=String(value||'').trim();if(!normalized||processing)return;const now=Date.now();if(normalized===lastValue&&now-lastAt<2500)return;lastValue=normalized;lastAt=now;processing=true;setResult('idle','CHECKING','Validating ticket and waiver…');try{const data=await api({action:'check-in',value:normalized});const packages=entitlementLabel(data.entitlements);if(data.result==='valid'){setResult('valid','VALID — CHECKED IN',`${data.guestName||'Guest'} · ${data.ticketId||''}${packages}`)}else if(data.result==='already_used'){setResult('used','ALREADY CHECKED IN',`${data.guestName||'Guest'}${data.checkedInAt?` · ${formatTime(data.checkedInAt)}`:''}${packages}`)}else{setResult('invalid','INVALID TICKET',data.message||'Ticket could not be validated.')}}catch(error){const data=error.data||{};if(data.code==='waiver_required'){setResult('invalid','WAIVER REQUIRED','Guest must sign the individual waiver before entry. Open the waiver link from Emergency Guest Lookup or their digital ticket.')}else if(data.result==='invalid'){setResult('invalid','INVALID TICKET',data.message||'Ticket could not be validated.')}else if(error.message==='Unauthorized.'){await stopScanner();showLogin();loginStatus.textContent='Check-in session expired. Sign in again.'}else setResult('invalid','CHECK-IN ERROR',error.message)}finally{processing=false}}

function escapeHtml(value=''){return String(value).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#039;')}
function renderLookup(matches=[]){if(!lookupResults)return;if(!matches.length){lookupResults.innerHTML='<p class="checkin-help">No active paid or comp ticket matched that search.</p>';return}lookupResults.innerHTML=matches.map((item,index)=>`<article class="checkin-lookup-card"><strong>${escapeHtml(item.guestName||'Guest')}</strong><span>${escapeHtml(item.email||'')} · ${escapeHtml(item.ticketSource==='comp'?'COMP':'PAID')}</span><span>${item.waiverSigned?'WAIVER SIGNED ✓':'WAIVER REQUIRED'}${item.checkedInAt?` · CHECKED IN ${escapeHtml(formatTime(item.checkedInAt))}`:''}</span><div class="checkin-actions"><button type="button" class="btn secondary" data-lookup-checkin="${index}" ${item.waiverSigned?'':'disabled'}>Check In</button><a class="btn secondary" href="${escapeHtml(item.ticketUrl)}" target="_blank" rel="noopener">Open Ticket</a>${item.waiverSigned?'':`<a class="btn" href="${escapeHtml(item.waiverUrl)}" target="_blank" rel="noopener">Sign Waiver</a>`}</div></article>`).join('');for(const button of lookupResults.querySelectorAll('[data-lookup-checkin]')){button.addEventListener('click',()=>validateTicket(matches[Number(button.dataset.lookupCheckin)]?.ticketUrl||''))}}
async function emergencyLookup(event){event.preventDefault();const query=String(lookupInput?.value||'').trim();if(query.length<3)return;lookupResults.innerHTML='<p class="checkin-help">Searching active tickets…</p>';try{const data=await api({action:'lookup-guest',query});renderLookup(data.matches||[])}catch(error){lookupResults.innerHTML=`<p class="checkin-help">${escapeHtml(error.message||'Lookup failed.')}</p>`}}
...
lookupForm?.addEventListener('submit',emergencyLookup);
