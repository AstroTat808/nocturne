...
function page({ paid = false, pending = false, ticketId = '', amount = '', digitalTicketUrl = '', message = '', drinkPackagePurchased = false } = {}) {
...
export default async (req) => {
  ...
  if (!order) { html = page({ pending: true }); status = 202; }
    ...
    html = page({ paid: true, ticketId: order.ticketId || 'Confirmed', amount, digitalTicketUrl, drinkPackagePurchased: Boolean(order.drinkPackageRequested || order.drinkPackagePurchased) });
