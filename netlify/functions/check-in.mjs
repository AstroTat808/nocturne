...
function key() {
  return process.env.NOCTURNE_CHECKIN_KEY || process.env.NOCTURNE_ADMIN_KEY || '';
}

function secret() {
  return process.env.NOCTURNE_CHECKIN_SESSION_SECRET || process.env.NOCTURNE_ADMIN_SESSION_SECRET || process.env.NOCTURNE_ADMIN_KEY || '';
}
...
function makeSession() {
  ...
  const payload = Buffer.from(JSON.stringify({ role: 'checkin', iat: now, exp: now + TTL_SECONDS, nonce: randomBytes(10).toString('base64url') })).toString('base64url');
...
function authenticated(req) {
  ...
  try {
    ...
    return data.role === 'checkin' && Number(data.exp) > Math.floor(Date.now() / 1000);
  } catch { return false; }
}
...
async function checkIn(req, input) {
  ...
  const summaryEntry = await orderStore.getWithMetadata(summaryKey, { type: 'json', consistency: 'strong' });
  const summary = summaryEntry?.data;

  if (!summary || summary.status !== 'paid' || summary.ticketId !== parsed.ticketId) {
    return json({ ok: false, result: 'invalid', message: 'INVALID OR INACTIVE TICKET' }, 403);
  }

  const application = await applicationStore.get(parsed.submissionId, { type: 'json', consistency: 'strong' });
  ...
  const write = await orderStore.setJSON(summaryKey, {
    ...summary,
    status: 'paid',
    checkedInAt,
  ...
  if (!write.modified) {
    ...
    return json({
      ok: true,
      result: latest?.checkedInAt ? 'already_used' : 'conflict',
  ...
  try {
    ...
    const review = await reviewStore.get(parsed.submissionId, { type: 'json', consistency: 'strong' });
...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    return authenticated(req) ? json({ authenticated: true }) : json({ authenticated: false }, 401);
  }
  ...
  if (action === 'login') {
    if (!key() || !secret()) return json({ error: 'Check-in authentication is not configured.' }, 500);
    if (!safeEqual(input.password || '', key())) return json({ error: 'Invalid check-in password.' }, 401);
    return json({ ok: true }, 200, { 'Set-Cookie': setCookie(makeSession()) });
  }
