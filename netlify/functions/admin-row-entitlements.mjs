...
import { lateStayConfig } from './_late-stay.mjs';
...
function parseCookies(req) { const header = req.headers.get('cookie') || ''; return Object.fromEntries(header.split(';').map((part) => { const index = part.indexOf('='); if (index < 0) return ['', '']; let value = part.slice(index + 1).trim(); try { value = decodeURIComponent(value); } catch {} return [part.slice(0, index).trim(), value]; }).filter(([key]) => key)); }
function authenticated(req) { if (!sessionSecret()) return false; const token = parseCookies(req)[SESSION_COOKIE]; if (!token || !token.includes('.')) return false; const [payload, signature] = token.split('.', 2); if (!safeEqual(signature, sign(payload))) return false; try { const data = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')); return data.role === 'admin' && Number(data.exp) > Math.floor(Date.now() / 1000); } catch { return false; } }
function ticketState(summary, review) { if (summary?.status === 'disputed' || review?.ticketState === 'disputed') return 'disputed'; if (summary?.status === 'refunded' || review?.ticketState === 'refunded') return 'refunded'; if (summary?.checkedInAt || review?.checkedInAt || review?.ticketState === 'checked_in') return 'checked_in'; if (summary?.status === 'paid' || review?.ticketState === 'paid') return 'paid'; if (summary?.status === 'checkout_created') return 'checkout_created'; if (summary?.status === 'checkout_expired') return 'checkout_expired'; return 'none'; }
function activeEntitlement(purchased, status) { if (!purchased) return false; return !['revoked', 'refunded', 'forfeited', 'invalidated', 'none'].includes(String(status || '').toLowerCase()); }
function paidEntitlement(purchased, status, purchaseType, checkoutStatus, ticketStateValue) { if (!purchased || !activeEntitlement(purchased, status)) return false; return purchaseType === 'addon' ? checkoutStatus === 'paid' : ['paid', 'checked_in'].includes(ticketStateValue); }
...
export default async (req) => {
  ...
  try {
    ...
    const drinkPrice = drinkPackageConfig().priceCents;
    const waterPrice = waterPackageConfig().priceCents;
    const lateStayPrice = lateStayConfig().priceCents;
    ...
    const entries = (await Promise.all(blobs.map(async ({ key }) => {
      ...
      const drinkPurchaseType = summary?.drinkPackagePurchaseType || review?.drinkPackagePurchaseType || null;
      const drinkCheckoutStatus = summary?.drinkPackageCheckoutStatus || review?.drinkPackageCheckoutStatus || null;
      ...
      const waterPurchaseType = summary?.waterPackagePurchaseType || review?.waterPackagePurchaseType || null;
      const waterCheckoutStatus = summary?.waterPackageCheckoutStatus || review?.waterPackageCheckoutStatus || null;
      ...
      const lateStayPurchaseType = summary?.lateStayPurchaseType || review?.lateStayPurchaseType || null;
      const lateStayCheckoutStatus = summary?.lateStayCheckoutStatus || review?.lateStayCheckoutStatus || null;
      const drinkPaid = paidEntitlement(drinkPurchased, drinkStatus, drinkPurchaseType, drinkCheckoutStatus, state);
      const waterPaid = paidEntitlement(waterPurchased, waterStatus, waterPurchaseType, waterCheckoutStatus, state);
