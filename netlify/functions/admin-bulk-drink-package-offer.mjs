...
const APPLICATION_STORE='nocturne-applications',REVIEW_STORE='nocturne-application-reviews',ORDER_STORE='nocturne-ticket-orders',EMAIL_EVENT_STORE='nocturne-email-events',SESSION_COOKIE='nocturne_admin',CAMPAIGN_VERSION='v4';
...
function offerState(summary,review,configs){
  ...
  const offerLateStay=Boolean(configs.lateStay.enabled&&!configs.lateStay.soldOut&&!summary.lateStayPurchased);
...
async function sendOffer(req,application,summary,submissionId,dateKey,configs,offers){
  ...
  if(offers.offerLateStay)lines.push(`Late Checkout / Car Camping — ${lateStayPrice}`,`Stay on the property after the 3:00 AM event end until 8:00 AM. Limited to ${configs.lateStay.capacity} guests; ${configs.lateStay.remaining} spots are currently available. Each person staying after 3:00 AM needs their own add-on.`,'');
  const text=[`${name},`,'','Your NOCTURNE ticket is confirmed. These optional add-ons are still available for your ticket.','',...lines,'Add-on purchases are final sale and non-transferable where stated.','',`Review available add-ons: ${ticketUrl}`,'',`Unsubscribe from promotional NOCTURNE updates: ${unsubscribeUrl}`,'','NOCTURNE Festival'].join('\n');
  const cards=[offers.offerDrink?`<p><strong>Six-Drink Package — ${esc(drinkPrice)}</strong><br>Six prepaid drink credits. Beer/well use one credit; premium uses one credit plus $5.</p>`:'',offers.offerWater?`<p><strong>Unlimited Drinking Water — ${esc(waterPrice)}</strong><br>Unlimited drinking water during festival operating hours.</p>`:'',offers.offerLateStay?`<p><strong>Late Checkout / Car Camping — ${esc(lateStayPrice)}</strong><br>Stay after the 3:00 AM event end until 8:00 AM. Limited to ${Number(configs.lateStay.capacity)} guests; ${Number(configs.lateStay.remaining)} spots are currently available. Each person staying after 3:00 AM needs this add-on on their own ticket.</p>`:''].join('');
  const html=`<!doctype html><html><body style="margin:0;background:#030303;color:#f7efe3;font-family:Arial,sans-serif"><div style="max-width:640px;margin:auto;padding:44px 24px"><div style="border:1px solid rgba(216,154,43,.35);background:#080604;padding:38px 30px"><div style="color:#d89a2b;font-size:11px;letter-spacing:3px;text-transform:uppercase">NOCTURNE · Ticket Add-Ons</div><h1 style="font-family:Georgia,serif;font-weight:400;color:#fff3df">Optional add-ons for your ticket.</h1><p style="color:#c8baa4;line-height:1.7">${esc(name)}, your NOCTURNE ticket is confirmed. These optional add-ons are still available.</p>${cards}<p style="color:#ffca61"><strong>FINAL SALE:</strong> Add-on purchases are non-refundable and attached to the registered ticket where stated.</p><p style="text-align:center"><a href="${esc(ticketUrl)}" style="display:inline-block;padding:14px 20px;background:#d89a2b;color:#0b0803;text-decoration:none">View Available Add-Ons</a></p><p style="margin-top:34px;color:#74695b;font-size:11px">Prefer not to receive promotional NOCTURNE updates? <a href="${esc(unsubscribeUrl)}" style="color:#9fbfd0">Unsubscribe</a>. Essential ticket and event-access emails will continue.</p></div></div></body></html>`;
...
export default async(req)=>{
  ...
  const summary={dateKey,campaignVersion:CAMPAIGN_VERSION,scanned:blobs.length,sent:counts.sent||0,multiOffers:counts.multi,drinkOnly:counts.drink,waterOnly:counts.water,lateStayOnly:counts.lateStay,lateStayRemaining:configs.lateStay.remaining,lateStaySoldOut:configs.lateStay.soldOut,duplicate:counts.duplicate||0,suppressed:counts.suppressed||0,ineligible:counts.ineligible||0,failed:counts.failed||0};
