...
function statusCard(label, purchased, pendingUrl) {
  ...
  if (pendingUrl) return `<div class="addon-status addon-pending"><strong>${escapeHtml(label)}</strong><span>Checkout already open</span><a class="btn secondary" href="${escapeHtml(pendingUrl)}">Resume Checkout →</a></div>`;
  return '';
}
...
function page({ token, guestName, ticketId, drink, water, late, message = '' }) {
  ...
  const option = (item, name, title, body) => item.available ? `<label class="drink-package-option addon-choice"><input type="checkbox" name="${name}" value="yes"><span><strong>${escapeHtml(title)} · ${money(item.priceCents)}</strong><small>${escapeHtml(body)}</small></span></label>` : '';
...
export default async (req) => {
  ...
  const drink = { purchased: Boolean(summary.drinkPackagePurchased), pendingUrl: drinkPending, priceCents: drinkConfig.priceCents, available: !drinkPending && drinkConfig.enabled && stripeReady && drinkPackageAddonEligible(summary, review, parsed.ticketId) };
  const water = { purchased: Boolean(summary.waterPackagePurchased), pendingUrl: waterPending, priceCents: waterConfig.priceCents, available: !waterPending && waterConfig.enabled && stripeReady && waterPackageAddonEligible(summary, review, parsed.ticketId) };
  const late = { purchased: Boolean(summary.lateStayPurchased), pendingUrl: latePending, priceCents: lateConfig.priceCents, available: !latePending && lateConfig.enabled && stripeReady && lateStayAddonEligible(summary, review, parsed.ticketId) };
